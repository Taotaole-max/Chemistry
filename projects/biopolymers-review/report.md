...
Biopolymers derived from renewable resources — polysaccharides, proteins, polyesters, lignin and
natural rubber — are candidate replacements for petrochemical plastics, but their uptake is limited
by properties that trace to backbone chemistry. This review organises the major families by their
repeating backbone linkage rather than by biological origin, and treats each material
property-and-use first, structure second. Three patterns hold across the families: the performance
ceiling is thermal and hygroscopic, not mechanical; crystallinity governs a three-way trade-off in
which the feature that supplies stiffness also blocks water uptake and the attack that degrades the
material; and molecular-weight data are comparable only alongside their measurement method. A
natural-rubber-versus-SBR case study shows the same backbone-versus-formulation trade-off within
one application. All figures are generated by code from cited data and machine-verified structures.
...
The review runs from classification (§2) and the family-by-family survey (§3) through molecular
weight (§4), the property patterns common to all families (§5), processing (§6), the comparison
with synthetic polymers (§7), applications (§8) and outlook (§9).
...
better" story does not survive the literature, though: NR latex carries a real, SBR-absent allergy
risk [27,28], renewability does not automatically mean a lower LCA footprint [29,30], and NR
pricing has been the more volatile of the two for climate and disease reasons unrelated to polymer
chemistry [32].
